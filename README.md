...
available NPU, CUDA device, or CPU in that order.

## Public boundary

Large models, restricted datasets, credentials, and run outputs stay outside
the repository. Set `IIRA2_DATA_ROOT`, `IIRA2_MODEL_ROOT`, and
`IIRA2_OUTPUT_ROOT` in the execution environment. No private absolute path is
required by the code or documentation.
